...
int rtsp_client_setup_onreply(struct rtsp_client_t* rtsp, void* parser)
{
	...
	if (200 == code)
	{
		...
		if (rtsp->media_count == ++rtsp->progress)
		{
			...
			return rtsp->handler.onsetup(rtsp->param, rtsp->session[0].timeout / 1000, RTSP_RANGE_TIME_NORMAL==range->from_value && RTSP_RANGE_TIME_NORMAL==range->to_value ? range->to - range->from : -1);
		}
		else
		{
			// setup next media
