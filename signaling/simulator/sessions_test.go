...
func TestSessionTableWaitsForKeyframeAndRemovesDensely(t *testing.T) {
	...
	if got := table.active(1, 1); len(got) != 1 || got[0] != 3 {
		t.Fatalf("worker 1 phase 1 = %v", got)
	}
	if !table.remove(1) || table.remove(1) {
		t.Fatal("remove result is wrong")
	}
	if table.activeCount() != 3 || len(table.active(1, 0)) != 0 {
		t.Fatalf("after remove active=%d bucket=%v", table.activeCount(), table.active(1, 0))
	}
