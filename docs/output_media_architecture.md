...
但新程序吞吐低约 6.3%，不能解释为稳态收益。24-worker 建连时间
38.1→46.5 秒，也提示同机负载/调度波动；WHEP 本轮新增的计数与匹配只发生在
建连/退出，不在帧发送热路径。保留此负面结果，不拿 950 路推断容量。
...
`.cache/output-whep-{before,after}-950-measured/`；每次的 `result.json`、
