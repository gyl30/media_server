...
{
    configuration config{{}, "", "127.0.0.1", 8554, 0, std::chrono::seconds{10}, 50};
...
boost::asio::awaitable<void> run_viewer(std::shared_ptr<run_state> state, std::size_t index)
{
    ...
    else
    {
        ...
        auto client =
            std::make_shared<media_server::test::rtsp_test_client>(state->io, "/" + state->config.stream_name + "?stream_id=" + allocation.stream_id);
