...
wait_event_state() {
    ...
    for _ in $(seq 1 100); do
        if curl --noproxy '*' -fsS --connect-timeout 1 --max-time 2 \
            ...
            python3 - "$response" "$stream_id" "$kind" "$protocol" "$stream_name" "$state" "$source_id" <<'PY'
import json
import sys
...
publish_pid=""
wait_event_state "$main_signaling_http_port" "$main_publish_stream_id" publisher rtmp live/test stopped
...
for publish_case in tcp udp udp-restart; do
    ...
    rtsp_publish_pid=""
    wait_event_state "$main_signaling_http_port" "$rtsp_publish_stream_id" publisher rtsp "live/$stream_name" stopped
    sleep 0.2
    kill -0 "$main_pid"
done
...
for transport in tcp udp; do
    ...
    wait "$rtsp_publish_pid" 2>/dev/null || true
    rtsp_publish_pid=""
    wait_event_state "$av1_signaling_http_port" "$rtsp_publish_stream_id" publisher rtsp "live/$stream_name" stopped
    sleep 0.2
    kill -0 "$av1_server_pid"
done
...
av1_publish_pid=""
wait_event_state "$av1_signaling_http_port" "$av1_publish_stream_id" publisher rtmp live/av1 stopped
