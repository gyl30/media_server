...
{
    configuration config{{}, "", 0, std::chrono::seconds{10}, 50};
...
boost::asio::awaitable<allocation_result> allocate(const run_state& state)
{
    ...
    try
    {
        ...
        const auto play_url = value.at("play_url").as_string();
        co_return allocation_result{{}, std::string(play_url.c_str(), play_url.size()), response.result_int(), {}};
    }
...
boost::asio::awaitable<void> run_viewer(std::shared_ptr<run_state> state, std::size_t index)
{
    ...
    else
    {
        ...
        auto client = std::make_shared<media_server::test::http_flv_test_client>(state->io);
        const auto play_error = co_await client->play(allocation.play_url);
        if (play_error)
        {
        ...
        else
        {
            ...
            while (!state->pre_measurement_abort)
            {
                if (!measurement_snapshot_taken && state->measurement_started)
                {
                    ...
                    measurement_timer.async_wait(
                        [client, measurement_expired](const boost::system::error_code& error)
                        {
                            if (!error)
                            {
...
int main(int argc, char** argv)
{
    ...
    {
        std::cerr << "fanout HTTP-FLV generator failed: " << error.what() << '\n';
        return 2;
    }
}
